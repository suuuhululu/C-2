...
현재 개발 기준은 [3개 노드의 인터페이스 안내](INTERFACE_GUIDE.md)와 [draw.io 시스템 아키텍처](architecture/README.md)다. 이전 개발 일지의 Clay 설명은 당시의 기록으로 보존하며 현재 실행 구조로 사용하지 않는다. 삭제의 원격 게시·병합 여부는 이 변경을 포함한 브랜치·PR의 상태로 판단한다.
