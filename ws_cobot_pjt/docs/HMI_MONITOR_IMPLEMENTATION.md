...
> **9/18~20 초기 구현 기록:** 아래의 당시 연결 상태·미구현 항목은 2026-09-23 `main`에 적용하지 않는다. 현재는 [백엔드](../backend/README.md), [프런트엔드](../frontend/README.md), [작업 흐름](INTERFACE_GUIDE.md)을 따른다.
...
5. 공정 발행자에서 ProcessState의 신호별 품질·실제 측정 시각을 채운다. 게이트웨이의 Time↔RFC3339, 품질에 따른 null, 비유한 숫자 정규화는 9/19 구현·시험했다. 발행자의 신선도·배열·프레임 검사와 HMI의 실제 장비 표시 연결은 후속 범위다.
...
- 위 9/18 시험은 임시 DB와 가짜 상대를 사용했다. 9/19 공통 타입의 Jazzy 빌드·직렬화와 게이트웨이 변환을 추가 확인했다. [후속 검증 기록](validation/2026-09-19-c2-interfaces.md). 실제 상대 통합·하드웨어 시험은 수행하지 않았다.
