...
문서 작성 기준 main은 `7374507`이다. PR #7로 추가된 `clay_carving`·`clay_hmi`와 PR #9의 [기존 실행 안내](../ws_cobot1/doc/clay_run.md)가 있다. 이 문서의 `c2_interfaces`·`c2_path`·`c2_process` 및 `/c2/*` 계약은 아직 구현되지 않았다. 기존 코드의 Topic·Service와 아래 계약이 자동으로 호환되는 것은 아니다.

9/18 후속 반영으로 세 패키지의 [개발 폴더·역할 안내](../ws_cobot1/src/README.md)를 추가했다. 현재는 README와 빈 폴더 보존용 `.gitkeep`만 있으며, 빌드 설정·메시지·노드 구현 완료를 뜻하지 않는다. 팀원과 AI의 공통 개발 규칙은 [AGENTS.md](../../AGENTS.md)를 따른다.

기존 코드의 로봇 동작·SVG 처리·화면 기능은 이후 구현 PR에서 재사용 여부를 검토한다. 이번 변경은 문서이며 패키지 이름 변경·소스 이동·실행 방식 전환은 수행하지 않는다. 기존 실행 안내의 시험 보고도 새 3개 노드 구조의 검증 결과로 옮겨 적지 않는다.
