...
- 양초 중심·반지름·윗면·바닥·작업 높이 범위는 매 준비의 실측 결과로 실행 후보 스냅샷에 기록한다. 반면 속도·가속도·가공 깊이·관절 한계와 `execution_context.entry_planning`은 배포 실행 프로파일의 승인 정책이다. 사전 검사는 두 입력 JSON을 수정하지 않고 형식·교차 참조만 확인하며, 측정 후 새 불변 스냅샷이 둘을 결합한다.
