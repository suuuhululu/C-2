...
> 2026-09-19 적용 범위: 아래 9/17 확장 흐름은 보관 기록이다. 자동 집기·청소·반납·그리퍼 열기 단계는 현재 실행 절차가 아니다. 철사 고정 중에는 금지한다. 현재 PRECHECK→TOOL_CHECK→조각·이탈→FINISH 흐름은 [고정 드릴 결정](C2_FIXED_DRILL_20260919.md)을 따른다.

> 2026-09-18 범위 갱신: 현재 구현할 축소 흐름은 [3개 노드 인터페이스 안내](INTERFACE_GUIDE.md)를 따른다. 시스템 모니터에서 도안을 입력하며 작업대·대상은 등록된 고정 좌표를 사용한다. 아래에 남긴 자동 위치·치수 확인 등의 이전 제안은 이번 구현 범위에 자동 포함하지 않는다.
