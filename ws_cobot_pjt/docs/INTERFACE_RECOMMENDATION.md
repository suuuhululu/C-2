...
> **9/20 설계 변경 검토:** 이 문서의 v2는 현재 공통 타입 기준이다. 사용자 요청에 따른 준비 확인→양초 실측→새 실행 경로→최종 관절 검사 흐름은 [변경안과 계약 대기 항목](INTERFACE_GUIDE.md)에 구분했다. 새 경로 식별·보정 상태·운영자 드릴 확인은 아직 통신 계약에 반영되지 않았다. 아래 기존 PRECHECK/TOOL_CHECK 순서를 새 흐름의 구현 완료로 해석하지 않는다. 이번 문서화에서는 타입·schema_version을 변경하지 않는다.
