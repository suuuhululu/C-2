...
> **2026-09-21 준비된 실행 정책:** 아래 PRECHECK의 기본 상태·제어권·정지
> latch·TCP/하중 확인은 준비되지 않은 직접 실행에 대한 일반 권장안이다.
> BIND가 성공한 REAL 작업은 [준비 Action 공통 구현 기준](PREPARE_WORKPIECE_ACTION.md)에
> 따라 실행 직전 해당 항목을 다시 조회하지 않는다. 준비 성공 기록과 동일
> snapshot binding, 경로·설정 무결성, 실제 실행 계획의 최종 IK·관절 검사를
> 확인하며 취소·정지 처리는 유지한다.
