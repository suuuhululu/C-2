...
기준: `main` `987a3b7` (2026-09-25, PR #90 병합). 현재 코드의 구조를 설명한다. 장치·실기 검증의 범위는 [인터페이스 안내](INTERFACE_GUIDE.md), [Interface Specification](INTERFACE_SPECIFICATION.md)과 관련 시험 기록을 따른다.
...
4. `ExecuteProcess`는 별도 요청이다. HMI와 공정이 모드·현재 준비·경로/설정 ID·버전·해시·상태를 검사한다. REAL 또는 승인된 entry 활성 prepared 정상 공정은 `PRECHECK → ENTRY → ENGRAVE → RETURN_HOME → FINISH`로 진행하며, 승인된 entry/복귀 정책과 실측 기하로 현재/HOME에서 첫 APPROACH까지의 계획과 조각 후 HOME 복귀 계획을 먼저 확정한다. entry 비활성 호환 흐름에서는 두 콜백이 생략될 수 있다. 이어 깊이·접근·이탈이 반영된 본 실행계획의 명시 waypoint IK/관절 검사를 통과한 경우에만 같은 계획으로 조각하고 검사된 복귀 계획을 실행한다. 실패·취소·정지 미확인 뒤 자동 복귀하지 않는다.
