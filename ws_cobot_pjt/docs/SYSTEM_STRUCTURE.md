...
준비 BIND가 성공한 작업의 실행은 동일 준비 성공 기록과 snapshot binding을
사용한다. 실행 직전에 측정 유효성·기본 로봇 상태·제어권·TCP/하중을 다시
조회하지 않는다. 경로·설정 무결성, 깊이·접촉 보정 뒤 실제 실행 계획의 최종
IK·관절 검사, 취소·정지와 실제 정지 확인은 유지한다.
...
6. HMI/서버 → **ExecuteProcess Action** → 김세은의 동일 준비 성공 기록·snapshot binding 확인과 `check_path_joints()` 최종 검사. 준비된 실행에서 기본 상태·제어권·TCP/하중을 다시 조회하지 않는다. 깊이·접근/이탈을 확정한 경로가 검사 대상이다.
