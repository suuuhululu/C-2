...
| `state_machine.py` | 함수 호출 순서·운영자 확인 대기·성공/실패/정지 전이 | main 없음, 담당자 작업 보고. 측정→경로 생성→미리보기→조각 요청 흐름 연결 예정 |
...
| `workpiece_calibration.py` | 실제 양초 중심·높이 등 측정·결과 전달. 기울기는 지원·검증 범위 명시 | 필수 추가 모듈, 공통 구현 예정 |
| `prepared_path.py` | 기존 로컬 깊이 적용 실험 초안 | 필수 추가 대상에서 제외. 생성 후 중심·기울기 재보정은 하지 않음. 파일 삭제는 아님 |
...
깊이를 경로 생성 시 반영할지 별도 준비 계산에 반영할지는 한 곳으로 합의한다. 최종 미리보기·검사 전에 확정하고 조각 중 이중 적용하지 않는다. 새 중심 좌표를 적용하려고 `prepared_path.py`를 반드시 추가할 필요는 없다.

기존 장착 3점 보정은 도구 설정 준비로 유지한다. **양초 좌표 측정은 제거하지 않고 3D 경로 생성 앞으로 옮긴다.** [인터페이스 안내](INTERFACE_GUIDE.md)에 계약 변경 범위를 기록한다.
