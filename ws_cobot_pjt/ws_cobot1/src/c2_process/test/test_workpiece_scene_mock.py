"""9/21 고정 TCP 사례는 당시 설정으로 검증한다. 최신 설정은 test_eight_point_config.py에서 검사."""
import json
...
def test_recorded_start_and_reference_plans_pass_model():
    c=json.loads((Path(__file__).resolve().parents[1]/'test/fixtures/workpiece_config_before_8point_20260923.json').read_text());w=c['workcell']
