...
from c2_process.engraving import ExecutionContext, execute_path  # noqa: E402
from c2_process.cleaning import clean_tool  # noqa: E402
...
def sample_path(n_cut=30, arc_deg=40.0):
    ...
    return dict(schema_version=1, path_id="p-test", path_version=1, source_mode="SIMULATION", frame_id="dsr01_base",
...
def make_ctx(mode="force_touch", source="SIMULATION"):
    return ExecutionContext(
        ...
        tool_profile=dict(contact_mode=mode, tool_axis="+z", depth_mm=0.5, clearance_mm=10.0, touch_extra_mm=8.0,
                          touch_force_n=0.8, touch_speed_mm_s=1.5, frame_id="dsr01_base",
                          cleaning=dict(pose=[0.30, 0.10, 0.12, 0, 1, 0, 0], inward=[0, 1, 0], up=[0, 0, 1],
                                        approach_mm=15, wipe_down_mm=6, wipe_up_mm=20, motion_profile_id="travel")))
...
def test_clean_tool_once():
    ad = MockRobotAdapter(surface_fn=lambda p, d: 0.012)
    ctx = make_ctx()
    r = clean_tool(ctx.tool_profile, ctx, ad, return_pose=[0.4, 0.0, 0.2, 0, 1, 0, 0])
    assert r.ok, r
    assert [c["fn"] for c in ad.calls] == ["move", "move", "probe_touch", "move", "move", "move"]
