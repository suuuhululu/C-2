...
| [workpiece_calibration.py](c2_process/workpiece_calibration.py) | 이시율 | `measure_workpiece(adapter, workcell, profiles, context, on_progress=None)`: 홈 확인 → 윗면·옆면 8점 → 원 맞춤·결과 반환 | **PR #42로 코드 존재**, 홈 처리·실기 오류 수정은 PR #45. 모의 142건/ROS 모의 통과, 단독 실물은 윗면·8점·계산·후퇴 연속 2회 완료(270.394/299.295초). 밑면 오프셋 미확인으로 절대 윗면·바닥 Z는 null. [현재 검증 범위](WORKPIECE_CALIBRATION.md) |
