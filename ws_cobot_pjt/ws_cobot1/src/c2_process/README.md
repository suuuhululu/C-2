...
| [engraving.py](c2_process/engraving.py) | 이시율 | `execute_path()`로 세은이 검사한 동일 경로의 APPROACH/CUT/TRAVEL/RETRACT 실행, 진행·마지막 완료 구간·실패 반환. 중심 재이동 없음 | **main 존재**. 검사한 최종 경로와 실행 경로 일치 보강은 로컬 초안. force_touch 깊이 미적용·fixed_depth 실행 중 깊이 적용 방식 재정리 필요 |
| [workpiece_calibration.py](c2_process/workpiece_calibration.py) | 이시율 | `measure_workpiece(adapter, workcell, profiles, context, on_progress=None)`: 홈 확인 → 윗면·옆면 8점 → 원 맞춤·결과 반환 | **PR #42로 코드 존재**, 홈 처리·실기 오류 수정은 PR #45. 모의 126건/ROS 모의 통과, 실물은 윗면·첫 옆점까지. 밑면 오프셋 미확인으로 절대 윗면·바닥 Z는 null. [현재 검증 범위](WORKPIECE_CALIBRATION.md) |
