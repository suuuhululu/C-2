...
| `workpiece_calibration.py` **신규 제안** | 이시율 | 드릴 OFF에서 실제 양초를 찍어 중심·높이 등 좌표를 측정하고 경로 생성 전에 전달. 정밀 기울기·테이퍼 보정은 후속 범위 | **구현 예정**. 측정 계산·접촉·결과 생성은 시율, 호출·결과 전달은 세은. 함수 이름·인자는 합의 예정 |
...
1. 이시율: `workpiece_calibration.py`를 구현하고 실측 결과·재사용 도구 기준·이동/접촉/가공/정지 설정을 제공한다. 전체 실측 TipCalibration의 제공 상태와 SIMULATION 입력은 구분하고, 자료 미완성을 이유로 모의 연결을 늦추지 않는다. 기존 세 파일의 실행·정지 연결도 맞춘다.
