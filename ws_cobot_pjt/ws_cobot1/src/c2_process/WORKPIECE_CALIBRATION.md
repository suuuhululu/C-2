...
검증: 모의 시험 **126건** 통과. 여기에는 실제 측정 어댑터 + 가짜 장치로 전체 8점 완료,
...
그 뒤 수정본으로 진행한 실물 시험은 아래와 같다. **전체 실물 8점 완료는 아직 아니다.**
...
양초 절대 윗면 Z 정확도나 밑면 오프셋 검증이 아니다. 두 번째 옆점의 실제 이탈 원인은 미확인이다.
...
현장 사용자가 자리를 비운 뒤에는 추가 실물 이동을 실행하지 않았다. 돌아온 뒤 실행할
...
시간초과를 완료로 표시하지 않는다. 홈 경유/윗면/첫 옆점까지 실물 확인했지만, 전체 8점 시간과 변경 속도의 전체 구간 검증은 미완료다.
...
| REAL 서비스 I/O/관측/정지/접촉/IK·FK 어댑터 | 코드 + 가짜 I/O + 실제 ROS 이동/취소·정지 확인. 홈 경유·윗면·첫 옆점까지 확인, 전체 8점 성공 미완료 |
