...
적용한 결정 (2026-09-19 최종 조합):
  ...
  - 작업 범위 초과·원통 내부 통과·clearance 미달·금지 영역·설정 불일치는
    **비용이 아니라 금지 조건**으로 다룬다 (후보에서 제외).
...
def forbidden_reasons(theta_a, h_a, theta_b, h_b, clearance_m):
    ...
    for h in (h_a, h_b):
        if not (lo - 1e-9 <= h <= hi + 1e-9):
            bad.append("HEIGHT_OUT_OF_RANGE")
            break
...
def order_safety_cost(strokes, clearance_m, initial_order=None, max_pass=50):
    ...
    def total(order):
        ...
        for k in range(len(order) - 1):
            c = move_cost(strokes[order[k]], strokes[order[k + 1]], clearance_m)
            if c is None:
                return math.inf
            s += c
        return s

    forbidden = sum(1 for i in range(n) for j in range(n)
                    if i != j and move_cost(strokes[i], strokes[j], clearance_m) is None)

    init = list(initial_order) if initial_order else list(range(n))
    cost_init = total(init)

    # NN (안전비용 기준)
    unused = set(range(n))
    nn = [init[0]]
    unused.discard(init[0])
    while unused:
        cur = nn[-1]
        cands = [(move_cost(strokes[cur], strokes[j], clearance_m), j) for j in unused]
        cands = [(c, j) for c, j in cands if c is not None]
        if not cands:
            nn.extend(sorted(unused))
            break
        nn.append(min(cands)[1])
        unused.discard(nn[-1])

    best = nn if total(nn) < cost_init else init
    ...
    for _ in range(max_pass):
        ...
        if not improved:
            break
    return best, {"reordered": best != init, "cost_initial": round(cost_init, 6),
