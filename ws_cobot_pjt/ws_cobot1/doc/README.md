...
2026-09-19, 기준 main `23eec53`와 HMI PR #17 `970ec35`. 팀 공정은 `ws_cobot1`, 외부 로봇·그리퍼 환경은 `ws_dsr`로 분리한다. 기존 Clay 코드와 전용 실행 안내는 PR #16에서 로컬 보관 후 저장소에서 제거했다. 과거 실행·시험 기록은 [보관·복구 기록](../../docs/LEGACY_CLAY_ARCHIVE.md)의 고정 커밋 링크로 확인한다.
...
| `monitor_gateway_node` | `backend/app/ros_bridge.py`에 ROS 클라이언트 코드 구현. 공통 생성 타입·파일 해석 연동 전이며 ROS 통합 기동 미검증 |
| 조각·청소 | `feat/12-robot-adapter`의 `4b6416d`에 소스 존재. 기준 main과 이 HMI 브랜치에는 미포함 |
...
실제 `c2_interfaces` 생성 타입과 좌표 파일을 해석하는 `artifact_loader`가 아직 없어 ROS 통합 기동은 검증하지 않았다. [모니터 구현·남은 연동 순서](../../docs/HMI_MONITOR_IMPLEMENTATION.md)를 먼저 확인한다. 가짜 경로는 실제 상대 노드에 전달하지 않는다.
