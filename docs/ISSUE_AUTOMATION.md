...
이 문서는 2026-09-15 사용자 요청으로 작성한 자동화의 동작과 적용 절차다. **2026-09-16 main 생성·기본 브랜치 지정과 팀원 권한 확인을 마쳤으며, 첫 PR은 동료 검토 중이다. Issue 자동화의 원격 실행은 아직 검증하지 않았다.** 검토한 파일을 main에 병합한 뒤 적용한다. 작업 브랜치가 기본 브랜치로 지정되더라도 main이 아니면 이 자동화는 실행을 건너뛴다.
...
1. 완료: [Git 가이드](GIT_GUIDE.md)에 따라 main 생성·기본 브랜치 지정을 마쳤다. main 보호는 유지한다.
2. 완료: 팀원 3명의 초대 수락과 Write 권한을 확인했다. 중복 초대는 보내지 않았다.
3. 진행 중: [첫 PR #1](https://github.com/suuuhululu/C-2/pull/1)에서 양식·설정·스크립트·워크플로를 검토하고, 동료 승인·자동 검사 통과 후 main에 병합한다.
