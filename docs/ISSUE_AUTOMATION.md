...
이 문서는 2026-09-15 사용자 요청으로 작성한 자동화의 동작과 적용 절차다. **2026-09-16 작업 브랜치의 commit·push가 승인되었다. Issue 자동화의 원격 실행은 아직 검증하지 않았다.** 기본 브랜치 main 초기화와 검토한 파일의 병합 후 적용한다. 작업 브랜치가 기본 브랜치로 지정되더라도 main이 아니면 이 자동화는 실행을 건너뛴다.
...
팀장이 본인 계정을 확인했고, 나머지 세 계정은 공개 프로필의 존재를 확인했다. 저장소 협업자 초대 수락·Write 권한은 이 확인과 별개다. 실행할 때 실제 권한과 배정 가능 여부를 다시 조회한다.
...
1. GitHub Issues → New issue → **작업 · 실습 · 검증** 양식을 선택한다.
2. 프로젝트와 분야, 최초 마감, 완료 조건을 작성한다. 날짜는 `YYYY-MM-DD`다.
...
명령은 댓글 첫 글자부터 작성한다. 코드 블록·인용 안에 넣지 않는다. 명령 댓글을 수정해 재사용하지 말고 새 댓글을 작성한다. 날짜 명령은 한 줄로 작성한다. 예시 날짜는 실제 작업에 맞게 변경한다.
...
현재 상태의 원본은 `status:todo / doing / review / blocked / done` 라벨이다. Projects 보드는 아직 구성하지 않았으며 이 코드가 보드의 Status·날짜 필드를 바꾸지는 않는다. 우선 다음 Issue 검색을 북마크해 운영할 수 있다.
...
1. 사용자 검토를 마치고 [Git 가이드](GIT_GUIDE.md)의 빈 저장소 최초 초기화 문제를 먼저 해결한다. 이 자동화를 위해 main 보호를 우회하지 않는다.
2. 팀원 3명이 저장소 협업자 초대를 수락했는지 확인한다. 이 작업에서는 초대 메시지를 보내지 않았다.
3. 검토한 양식·설정·스크립트·워크플로를 작업 브랜치의 PR로 main에 반영한다.
4. Actions에서 **Issue management → Run workflow → main**을 한 번 실행한다. 이때 필요한 라벨이 생성된다.
5. 연습용 Issue 1개로 배정 → 시작 → 날짜 변경 요청 → 팀장 승인 → 검토 → 사람이 닫기까지 확인한다. 실제 확인 결과를 [준비 상태](REVIEW_STATUS.md)에 기록한다.
6. 처음 며칠은 팀장이 실행 결과와 배정량을 매일 확인한다. 로컬 검사의 성공을 GitHub 실행 성공으로 기록하지 않는다.
