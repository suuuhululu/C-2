...
외부 소스 제외 규칙은 PR #3으로 main에 반영됐다. 아래 메인 중심 구조와 양식 정리는 현재 로컬 브랜치의 미게시 변경이다. 원격 반영 여부는 [진행 현황](REVIEW_STATUS.md)을 확인한다.
...
1. 현재 main을 clone하거나 기존 작업을 보존한 뒤 병합된 변경을 받는다. 기존 강사 작업 폴더를 덮어쓰지 않는다. 로컬 정리안은 해당 PR 병합 후 공유된다.
