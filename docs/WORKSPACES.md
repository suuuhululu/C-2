...
2026-09-16 사용자 제공 강사 디렉토리 구조에 맞췄다. Git 저장소는 C-2 하나이며, 아직 실제 패키지·설치 프로그램·실행 환경은 준비하지 않았다.
...
각 폴더에서 새로 git init하지 않는다. 외부 원본은 [의존성 기록](DEPENDENCIES.md)에 따라 별도로 준비한다. 팀 코드는 고유한 패키지명으로 관리한다.

## 현재 준비 범위

- 강사 예시의 DoosanBootcamInt1·onrobot_rg2는 출처·버전이 제공되지 않아 설치하지 않았다. 이름만 있는 가짜 패키지를 만들지 않고 src 자리만 준비했다.
...
아래는 Ubuntu Jazzy와 필요한 소스·의존성이 준비된 이후의 경로 예시다. 현재 src에는 실행할 패키지가 없다. 실제 제어기 버전별 빌드 옵션은 제조사 문서와 대조한다.
