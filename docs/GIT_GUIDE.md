...
2026-09-16 게시 직전 원격과 로컬에 커밋이 없음을 다시 확인했다. `docs/workspace-setup`에서 빈 기준 커밋을 먼저 만들고 그 위에 프로젝트 파일을 커밋해 게시하는 순서로 준비한다.

빈 저장소에는 PR 비교 대상인 main 브랜치가 없다. 빈 기준 커밋과 프로젝트 변경 커밋을 같은 작업 브랜치의 이력으로 분리하면 이후 main을 빈 기준 커밋에 연결해 실제 변경을 PR로 비교할 수 있다. **main 브랜치 생성 방식은 보호 규칙과 함께 별도로 확인하며, 임의로 규칙을 우회하지 않는다.**

권장 순서:

1. 사용자 승인을 받은 작업 브랜치에 빈 기준 커밋과 프로젝트 변경을 나누어 게시한다.
2. 저장소 소유자와 main 생성 방식·보호 규칙과의 관계를 확인하고 빈 기준 커밋에 main을 준비한다. 첫 브랜치도 일반 PR로 만들어지는 것처럼 안내하지 않는다.
3. 기본 브랜치를 main으로 설정한 뒤 기존 작업 브랜치에서 main 대상 PR을 만든다.
4. 동료 1명 이상을 저장소 협업자로 등록하고 첫 PR을 검토·병합한다.
5. 자동 검사 실행을 확인한 뒤 필수 상태 검사 `repository-checks`를 보호 규칙에 추가한다.

현재 hook은 최초 `main` 생성 push도 차단한다. 초기화를 위해 임의로 hook이나 서버 규칙을 우회하지 않는다.
...
- CI를 GitHub에서 처음 실행하고 정상 체크 이름을 확인한 다음 필수 검사로 지정한다. 현재는 워크플로 파일만 초안으로 준비한다.
